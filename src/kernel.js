...
system = {
    ...
    help( args ) {
        return new Promise( ( resolve ) => {
            ...
            if ( args.length === 0 ) {
                ...
                resolve( [ "用法：", "> clear", "清屏命令会清除终端当前显示的内容，但不会影响历史记录。" ] );
            } else if ( args[ 0 ] === "date" ) {
                resolve( [ "用法：", "> date", "显示当前日期和时间。" ] );
            } else if ( args[ 0 ] === "echo" ) {
                resolve( [ "用法：", "> echo 内容", "在终端输出你输入的文本。" ] );
            } else if ( args[ 0 ] === "help" ) {
                resolve( [ "用法：", "> help", "显示帮助信息，列出当前服务器上可用的命令。" ] );
            } else if ( args[ 0 ] === "history" ) {
                resolve( [ "用法：", "> history", "显示你在本终端中输入过的所有命令历史。" ] );
            } else if ( args[ 0 ] === "login" ) {
                resolve( [ "用法：", "> login 用户名:密码", "切换账号：以其他已注册用户身份登录，以访问该用户的数据和邮件。" ] );
            } else if ( args[ 0 ] === "mail" ) {
                resolve( [ "用法：", "> mail", "登录后可查看你的收件箱邮件列表。" ] );
            } else if ( args[ 0 ] === "ping" ) {
                resolve( [
                    "用法：",
                    "> ping 地址",
                    "尝试连接指定地址的服务器。",
                    "如果没有返回有效响应，可能是地址错误、服务器不存在或无法从本地访问。"
                ] );
            } else if ( args[ 0 ] === "read" ) {
                resolve( [ "用法：", "> read 编号", "阅读指定编号的邮件。先用 mail 查看邮件列表，再用 read 编号 阅读。" ] );
            } else if ( args[ 0 ] === "ssh" ) {
                resolve( [
                    "用法：",
                    "> ssh 地址",
                ...
                resolve( [ "用法：", "> whoami", "显示当前连接的服务器和登录的用户名。" ] );
    ...
    read( args ) {
        return new Promise( ( resolve, reject ) => {
            ...
            message.push( `收件人：${ userDatabase.userId }@${ serverDatabase.terminalID }` );
            message.push( "---------------------------------------------" );
    ...
    ping( args ) {
        return new Promise( ( resolve, reject ) => {
            ...
            $.get( `config/network/${ args }/manifest.json`, ( serverInfo ) => {
                resolve( `服务器 ${ serverInfo.serverAddress }（${ serverInfo.serverName }）可以访问` );
            } )
    ...
    telnet() {
        return new Promise( ( _, reject ) => {
            reject( new Error( "telnet 不安全且已弃用——请使用 ssh" ) );
        } );
    },
